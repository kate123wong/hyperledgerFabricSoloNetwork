...
if [ "${MODE}" == "up" ]; then
  ...
  replacePrivateKey
  generateChannelArtifacts
  echo "BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org1.example.com/ca && ls *_sk)" > .env_key
  echo "BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org2.example.com/ca && ls *_sk)" >> .env_key
